...
export const bookMarkPost = async (req, res) => {
    try {
        ...
        const post = await PostModel.findById(postId);
        if (!post)
            return res.status(404).json({ message: 'Post which you want to bookmark/Unbookmark doesnt exists', success: false });
        const user = await UserModel.findById(userId);
        if (!user)
            return;
...
export const likePost = async (req, res) => {
    try {
        ...
        const post = await PostModel.findById(postId);
        if (!post)
            return res.status(404).json({ message: 'Post which you want to Like/disLike doesnt exists', success: false });
        const user = await UserModel.findById(userId);
        if (!user)
            return;
        // console.log(post);
